...
def start(message):
    mess = f'Привет, {message.from_user.first_name} {message.from_user.last_name}. Добро пожаловать в театр нового времени!\n\nМы находимся не только в Самаре. Выбери город чтобы фильтровать список представлений.\n\n Также ты можешь узнать о нас подробнее на нашем сайте.'
...
def get_text(message):
    ...
    if (confurm == 20):
        if (str(message.text) == str(int(new_client[0]) + pas)) and (unike_test(new_client, sheet_object=client_sheet) == 0):
            ...
            markup.add(types.InlineKeyboardButton("перейти", url='https://www.youtube.com/watch?v=84YxwrxRc5s&t=28s'))
    ...
    try:
        if call.message:
            ...
            if call.data == 'net_pus':
                ...
                markup1.add(types.InlineKeyboardButton("перейти", url='https://www.youtube.com/watch?v=84YxwrxRc5s&t=28s'))
